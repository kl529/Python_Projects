...
class fields:
	...
	def result(self):
		...
		if self.__bomb: # 폭탄을 냈을때
			...
			for k in range(12):
				if len(ffield[k])>0:
					if fnextcard.month == ffield[k][0]: # 덱에서 나온 카드와 같은 월이 필드에 존재한다.
						if len(ffield[k]) == 1: # 덱에서 나온카드와 같은 월이 1개일때 
							...
							response = Reader.card_choose(fobject) # 2개중 무엇을 먹을건지 선택
							ffield[k].append(fnextcard)
				...
				for i in range(12):
					if len(ffield[i])>0:
						if ffield[i][0].month == fput.month: #폭탄 3장 다내고 피 1장
							...
							steal += 1
							if fobject.name=="Computer":
								fobject.get(Card("폭탄", "13", False,"폭탄.png"))
								fobject.get(Card("폭탄", "13", False,"폭탄.png"))
							if fobject.name!="Computer":
								fobject.get(Card("폭탄", "13",True,"폭탄.png"))
								fobject.get(Card("폭탄", "13",True,"폭탄.png"))								
							fobject.shake
							bombcheck=False
							temp=True
								...
								if len(ffield[k])>0:
									if ffield[k][0].month==fnextcard.month:
										if len(ffield[k])==1:
											...
											if fobject.name!="Computer":
												print(ffield[k][0],"/",ffield[k][1],"중", end=" ")
											response = Reader.card_choose(fobject)
											ffield[k].append(fnextcard)
										...
										if len(ffield[k])>0:
											if ffield[k][0].month==fnextcard.month:
												if len(ffield[k])==1:
													...
													if fobject.name!="Computer":
														print(ffield[k][0],"/",ffield[k][1],"중", end=" ")
													response2 = Reader.card_choose(fobject)
													ffield[k].append(fnextcard)
